...
thread_unblock (struct thread *t) 
{
  ...
  /* Insert thread into ready list in priority order */
...
thread_set_priority (int new_priority) 
{
  ...
  /* Yield if we no longer have the highest priority */
  if (cur->priority < old_priority && !list_empty(&ready_list)) {
...
{
  /* Not implemented for basic priority scheduler */
}
...
/* Put current thread to sleep until wake_tick */
void
...
/* Check sleeping threads and wake up those whose time has come */
void
thread_check_sleeping (int64_t current_tick)
{
...
/* Comparison function for priority-ordered lists */
bool
...
/* Update thread's priority based on donations */
void
thread_priority_update (struct thread *t)
{
  ...
  if (!list_empty (&t->donations))
    {
      /* Find the maximum donated priority */
      struct list_elem *e;
...
/* Donate priority to thread holding a lock */
void
thread_priority_donate (struct thread *t)
{
  ...
  /* Handle nested donation up to 8 levels */
  while (lock != NULL && depth < 8)
    {
      ...
      /* Add current thread to holder's donation list if not already there */
      bool found = false;
      struct list_elem *e;
      ...
      thread_priority_update (holder);
      
      /* If priority didn't change, no need to continue */
...
/* Remove all donations related to a specific lock */
void
thread_priority_remove_lock (struct lock *lock)
{
  if (thread_mlfqs)
    return;
  ...
  /* Update priority after removing donations */
  thread_priority_update (cur);
