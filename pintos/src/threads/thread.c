...

/* Called when the current thread is about to wait on LOCK.
   Sets waiting_lock and donates priority to the lock holder if needed. */
void
thread_lock_will_wait (struct lock *lock)
{
  struct thread *cur = thread_current ();
  ...
  if (holder != NULL && cur->priority > holder->priority)
    {
      ...
      /* Check if holder is waiting on another lock. */
      if (holder->waiting_lock != NULL)
        {
          struct thread *next_holder = lock_get_holder (holder->waiting_lock);
          if (next_holder != NULL && max_priority > next_holder->priority)
            {
              /* Recursively donate up the chain. */
              struct lock *l = holder->waiting_lock;
              while (l != NULL && next_holder != NULL)
                {
                  if (max_priority > next_holder->priority)
                    {
                      int old_priority = next_holder->priority;
                      next_holder->priority = max_priority;
                      
                      /* If priority increased and thread is ready, re-insert. */
                      if (next_holder->status == THREAD_READY && 
                          old_priority < max_priority)
                        {
                          list_remove (&next_holder->elem);
                          list_insert_ordered (&ready_list, &next_holder->elem,
                                               thread_priority_less, NULL);
                        }
                    }
                  
                  l = next_holder->waiting_lock;
                  if (l != NULL)
                    next_holder = lock_get_holder (l);
                  else
                    break;
                }
            }
      ...
      if (max_priority > holder->priority)
        {
          int old_priority = holder->priority;
          holder->priority = max_priority;
          
          /* If priority increased and thread is ready, re-insert. */
          if (holder->status == THREAD_READY && old_priority < max_priority)
            {
...
/* Called when the current thread has acquired LOCK.
   Adds the lock to the thread's locks list. */
void
thread_lock_acquired (struct lock *lock)
{
  struct thread *cur = thread_current ();
...
/* Called when the current thread has released LOCK.
   Removes the lock from the thread's locks list and recalculates priority. */
void
thread_lock_released (struct lock *lock)
{
  struct thread *cur = thread_current ();
  ...
  /* Recalculate priority based on original priority and remaining locks. */
  max_priority = cur->original_priority;
  for (e = list_begin (&cur->locks); e != list_end (&cur->locks);
       e = list_next (e))
    {
      struct lock *l = list_entry (e, struct lock, elem);
      struct thread *waiter = lock_get_highest_priority_waiting_thread (l);
      if (waiter != NULL && waiter->priority > max_priority)
  ...
  if (max_priority != cur->priority)
    {
      ...
      /* If priority decreased and thread is ready, re-insert. */
      if (cur->status == THREAD_READY && old_priority > max_priority)
        {
